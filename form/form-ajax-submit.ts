...
export const onSubmit = (e: FormEvent, options: { beforeSend?: (form: HTMLFormElement) => (void | false), success?: (res: Response) => void, fail?: (err: Error) => void, complete?: () => void } = {}) => {
    ...
    const url = (form.action || window.location.href) + (method == 'GET'? `?${new URLSearchParams(formData as any).toString()}` : '');
    ...
    if (method != 'GET')
        switch (enctype) {
            ...
            default:
                ...
                init.body = new URLSearchParams(formData as any).toString();
                break;
        }
